...
## 2026-08-05：费用采集解耦为数据集驱动（D011）+ .env 生效 + 熔断全覆盖

- **为什么**：费用是独立功能，采集范围应由用户名单决定；此前目标筛选写死驳回状态（页面上费用菜单挨着发文菜单导致的历史耦合）。
- **做了什么**：新增 `fee_targets` 表与 `import_fee_targets.py`（CSV/Excel 导入即整表替换）、Dashboard 上传入口与数据集统计；`collect_fees.py` 默认采数据集内费用缺失的已建档记录，`--force` 重采整个数据集；删除驳回口径费用查询与 `detail_enrichment_*`。同日：`settings.py` 载入 `.env`（显式环境变量优先）；三个采集入口接入连续失败熔断（阈值共用 `WATCHDOG_FAILURE_THRESHOLD`）。
- **关键取舍**：数据集内未建档申请号不进待采队列（否则无处落库、反复失败触发熔断），单列计数提示先跑主采集；`fee_targets` 不参与 replica 同步与 JSONL 备份。
- **结果**：pytest 257 passed；导入→统计→采集→出队端到端已验证。
