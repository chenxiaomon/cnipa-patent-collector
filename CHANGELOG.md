...
### 修订 r2
- Phase 0 缓存导入与 MITM 写入使用同一跨进程锁；SQLite 和完整 JSONL 备份均成功后才清缓存，失败时保留缓存供重试。
- 原子 JSON 写入改用同目录唯一临时文件；Dashboard 同时阻止 Phase 0 浏览器与缓存导入任务，避免并发覆盖。
- replica 增量同步核对 HTTP `Content-Length`，响应缺失长度或传输截断时不写数据库、不刷新同步游标。
- JSONL 重建拒绝空快照，并与专利写入共享跨进程锁；在单个 SQLite 事务中精确替换专利表，保留费用目标、请求与采集失败记录，损坏数据库拒绝在线换文件。
- Dashboard 增量导入按数据库字段契约整批校验 JSON、申请号、字段类型和有限数字，并使用独立的 128 MB 上传限制。
- 看门狗强制主采集启用 MITM，并在创建新批次前检查代理；代理不可用时写入报警并停止。

### 部署 r2
- 更新后运行 `setup.bat`，重启 Dashboard 和主 MITM 代理，再启动看门狗。公司 Windows 电脑仍须按 `docs/runbook.md` 完成代理失败与小批量真实采集验收。

### 修订 r1
- 主采集、发文和费用采集新增本机持久批次历史，记录目标、失败原因及每轮执行情况；Dashboard 可筛选、下载记录，并续跑同一批次的未完成项，重启后仍可查看。
- 看门狗启动时冻结本次主采集批次；进程崩溃或健康检查重启后继续同一批未成功目标，并以持久批次状态判定完成。
- Dashboard 新增一键环境诊断及 JSON 报告，检查实际采集 Python、依赖、Chrome/驱动、代理、坐标和存储权限；仅反映运行 Dashboard 的电脑，不代表公司远程电脑状态。
- 主采集、发文和费用采集保留原子写入的未完成清单，仅成功落库后移除；浏览器退出、用户中断及收尾仍有失败项时不再报告正常完成，异常记录和历史异常记录可进入重试。
- 增量同步游标在 SQLite 写事务中分配，按提交顺序递增；一次性规范化旧时间格式，升级后自动全量对账一次，并提供 `--full` 和 Dashboard 全量对账入口。同一副本机的同步任务互斥。
- 日期版本保持旧更新器兼容，新增独立 `RELEASE_REVISION` 修订号；更新检查和安装校验识别同日修订并拒绝降级。
- Dashboard 显示正在运行与已安装的修订；更新或回滚后出现待重启状态。
- 首次安装 r1 需要手动触发 HTTP 更新并重启。公司 master 和开发 replica 均升级后，在 replica 执行一次全量对账，详见 `docs/runbook.md`。

### 修复
- Dashboard 筛选导出保留跨搜索的申请人选择，显示已选数量并支持清空；修复移动端勾选框尺寸和预览失败提示。
- 任务日志刷新保留手动阅读位置，回到底部后继续跟随新日志；切换任务和乱序响应不再覆盖较新的日志，慢响应仍可正常刷新。
...
- 代码更新和回滚全程持有跨进程维护锁，与看门狗、详情采集、Phase 0 及公开查询浏览器互斥；Dashboard 在生成手工批次文件前先拒绝冲突任务。
- 发文采集改用独立 `fwxx_collected_at`，不再刷新策略复查使用的案件状态 `timestamp`；旧数据库自动无损增加该字段。
- 筛选 Excel 生成失败返回明确错误并清理临时文件；任务日志保留所有活动任务，只裁剪最近结束的历史。
- 坐标录制、Dashboard 保存和环境诊断共用同一校验规则，拒绝错误类型与完整 `(0, 0)` 占位坐标。
